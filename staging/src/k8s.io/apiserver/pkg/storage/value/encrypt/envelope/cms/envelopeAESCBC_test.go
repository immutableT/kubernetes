...
var (
	...
	dek = []byte{91, 81, 103, 129, 167, 171, 49, 149, 157, 252, 192, 207, 202, 64, 7, 132, 96, 186, 94, 23, 59, 49, 8, 241, 199, 168, 138, 107, 7, 245, 13, 73,}
...
func TestCMSAESCBCParseFomOpenSSL(t *testing.T) {
	e, err := ParseEnvelope(referenceEnvelope)
	if err != nil {
	...
	if ! bytes.Equal(gotDEK, dek) {
		t.Fatalf("got %v, want %v", gotDEK, dek)
	}
...
func TestCMSAESCBCConstructor(t *testing.T) {
	...
	e, err := newEnvelopeAESCBC(want, dek, iv, kms)
	if err != nil {
...
func TestCMSAESCBCMarshal(t *testing.T) {
	e, err := newEnvelopeAESCBC([]byte("Hello CMS."), dek, iv, kms)
	if err != nil {
		t.Fatalf("failed to construct Envelope, err: %v", err)
	}
